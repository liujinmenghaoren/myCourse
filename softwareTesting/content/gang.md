...
白盒测试通过重点关注源代码中不同类型的结构，如判定表达式，执行路径，循环结构，数据变量等，引入不同的白盒覆盖指标，从而得到不同的白盒测试方法，这些方法侧重点不同，对应源代码结构的覆盖程度也不同。

**代码检查**

代码检查主要是通过同行评审方法来发现缺陷，即基于缺陷预防的思想，以评审会议的形式，通过多人对软件交付物进行检查，从而发现缺陷或获得改进优化的机会。

同行评审方法主要包括：审查，团队评审，走查，结对编程，同行桌查，轮查，特别检查。